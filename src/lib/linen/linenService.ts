...
export function calculateDotazioni(
  ...
  // 🔥 FIX: Se termine scaduto e ospiti NON confermati → usa maxGuests
  let guestsCount = cleaning.guestsCount || 2;
  ...
  debugLog('🔍', `Inventario: ${inventory.length} items. Primi 3:`, inventory.slice(0, 3).map(i => ({ id: i.id, name: (i as any).name, sellPrice: i.sellPrice, price: (i as any).price })));
  ...
  if (savedConfig) {
    ...
    if (savedConfig.bl) {
      ...
      // Nota: bl['all'] contiene i TOTALI configurati dall'utente - è la fonte di verità!
      // I gruppi per letto (b1, b2, etc) sono solo dettagli per la distribuzione
      
      const blKeys = Object.keys(savedConfig.bl);
      const bedGroupKeys = blKeys.filter(k => k !== 'all');
      const hasAll = blKeys.includes('all') && savedConfig.bl['all'] && Object.keys(savedConfig.bl['all']).length > 0;
      const hasBedGroups = bedGroupKeys.length > 0 && bedGroupKeys.some(k => {
        const items = savedConfig.bl[k];
      ...
      if (hasAll && hasBedGroups) {
        // 🔧 FIX: Merge - usa 'all' come base, poi integra dai gruppi letto gli articoli mancanti
        // bl['all'] è il totale configurato dall'utente MA spesso manca delle voci (es. lenzuola)
        // I gruppi letto hanno i dati completi per ogni letto
        const allItems = { ...savedConfig.bl['all'] };
        const allItemKeys = new Set(Object.keys(allItems));
        
        // Somma articoli dai gruppi letto che NON sono già in 'all'
        const bedGroupTotals: Record<string, number> = {};
        bedGroupKeys.forEach(k => {
          const groupItems = savedConfig.bl[k];
          if (groupItems && typeof groupItems === 'object') {
            Object.entries(groupItems).forEach(([itemId, qty]) => {
              if (typeof qty === 'number' && qty > 0) {
                bedGroupTotals[itemId] = (bedGroupTotals[itemId] || 0) + qty;
              }
            });
          }
        });
        
        // Integra in allItems solo gli articoli che mancano da 'all'
        Object.entries(bedGroupTotals).forEach(([itemId, qty]) => {
          if (!allItemKeys.has(itemId)) {
            allItems[itemId] = qty;
          }
        });
        
        blEntries = [['all', allItems]];
        debugLog('🛏️', `Usando bl['all'] integrato con gruppi letto. Risultato:`, allItems);
      } else if (hasAll) {
        blEntries = [['all', savedConfig.bl['all']]];
        debugLog('🛏️', `Usando bl['all'] (nessun gruppo letto disponibile)`);
      } else if (hasBedGroups) {
        blEntries = bedGroupKeys.map(k => [k, savedConfig.bl[k]]) as [string, Record<string, number>][];
        debugLog('🛏️', `Usando gruppi letto [${bedGroupKeys.join(', ')}] (no 'all' presente)`);
      } else {
        blEntries = [];
        debugLog('⚠️', `Nessun dato biancheria letto trovato in bl`);
      }
      ...
      blEntries.forEach(([groupKey, items]) => {
        ...
        Object.entries(items as Record<string, number>).forEach(([itemId, qty]) => {
          if (qty > 0) {
            // Traduci itemId → nome italiano per matching inventario
            const translatedName = getItemName(itemId);
            
            let invItem = inventory.find(i => 
              i.id === itemId || 
              (i as any).key === itemId ||
              ((i as any).name || '').toLowerCase().includes(itemId.toLowerCase()) ||
              (translatedName !== itemId && ((i as any).name || '').toLowerCase() === translatedName.toLowerCase())
            );
            ...
            // Usa nome inventario → nome tradotto → itemId come fallback
            const name = (invItem as any)?.name || (translatedName !== itemId ? translatedName : itemId);
...
export function configToSelectedItems(
  ...
  // 🔧 FIX: Se 'all' esiste e ha valori, usa SOLO quello (è la fonte di verità)
  // Altrimenti somma dai gruppi letto
  if (config.bl) {
    
    const bedLinenTotals: Record<string, number> = {};
    
    // Controlla se 'all' esiste e ha valori
    const allItems = config.bl['all'] || config.bl.all;
    const hasValidAll = allItems && 
                        typeof allItems === 'object' && 
    ...
    if (hasValidAll) {
      ...
      Object.entries(config.bl).forEach(([bedId, bedItems]) => {
        if (bedId === 'all') return; // Skip 'all' vuoto
    ...
    // 3. Ora crea gli items dall'inventario
    Object.entries(bedLinenTotals).forEach(([itemId, qty]) => {
      if (qty > 0) {
...
// ============================================================
// 🔧 FUNZIONI HELPER CENTRALIZZATE PER BIANCHERIA LETTO
// Da usare in TUTTI i file che leggono config.bl
// ============================================================

/**
 * 🔧 ESTRAE I TOTALI BIANCHERIA LETTO DA UNA CONFIG
 * 
 * Questa funzione implementa la logica CORRETTA per leggere bl:
 * - Se bl['all'] esiste e ha valori → usa SOLO quello (totali configurati dall'utente)
 * - Altrimenti → somma dai gruppi letto individuali (b1, b2, etc.) per retrocompatibilità
 * 
 * IMPORTANTE: Questa è la STESSA logica usata dal configuratore (getItemQty).
 * Usare questa funzione ovunque per garantire consistenza.
 * 
 * @param bl - L'oggetto bl dalla config (può essere undefined)
 * @returns Record<string, number> - I totali per ogni item { itemId: quantity }
 * 
 * @example
 * const totals = extractBedLinenTotals(config.bl);
 * // totals = { doubleSheets: 5, pillowcases: 2 }
 */
export function extractBedLinenTotals(bl: Record<string, Record<string, number>> | undefined): Record<string, number> {
  ...
  if (hasAll) {
    ...
    // 🔄 FALLBACK: Somma dai gruppi letto individuali (retrocompatibilità)
...
// ============================================================
// 🔧 FUNZIONE HELPER PER NORMALIZZARE CHIAVI serviceConfigs
// Firestore salva le chiavi come stringhe ("2") ma cerchiamo con numeri (2)
// ============================================================

/**
 * Normalizza l'accesso a serviceConfigs gestendo chiavi stringa/numero
 * USARE SEMPRE QUESTA FUNZIONE per accedere alle config!
 */
export function normalizeConfigKey(
  configs: Record<string | number, any> | undefined,
  guestsCount: number
): any {
  if (!configs) return undefined;
  // Prova prima con numero, poi con stringa
  return configs[guestsCount] || configs[String(guestsCount)] || undefined;
}

// ============================================================
// 🔧 FUNZIONE CENTRALIZZATA PER CALCOLARE ITEMS ORDINE
// Da usare in TUTTI gli endpoint che creano/aggiornano ordini
// ============================================================

const ORDER_ITEM_NAMES: Record<string, string> = {
...
function getOrderItemName(itemId: string): string {
  return ORDER_ITEM_NAMES[itemId] || itemId;
}

/**
 * 🔧 CALCOLA ITEMS PER ORDINE BIANCHERIA
 * 
 * Funzione centralizzata per calcolare gli items da salvare in un ordine.
 * USA SEMPRE bl['all'] se presente (è il totale configurato dall'utente).
 * 
 * @param serviceConfigs - Le configurazioni della proprietà
 * @param guestsCount - Il numero di ospiti
 * @returns Array di items per l'ordine
 */
export function calculateOrderItemsFromConfig(
  serviceConfigs: Record<string | number, any> | undefined,
  guestsCount: number
): { id: string; name: string; quantity: number }[] {
  const items: { id: string; name: string; quantity: number }[] = [];
  
  if (!serviceConfigs) {
    debugLog('⚠️', 'calculateOrderItems: serviceConfigs non presente');
    return items;
  }
  
  // Trova la config per questo numero di ospiti
  const config = normalizeConfigKey(serviceConfigs, guestsCount);
  
  if (!config) {
    debugLog('⚠️', `calculateOrderItems: nessuna config per ${guestsCount} ospiti`);
    return items;
  }
  
  debugLog('📦', `calculateOrderItems: Calcolando per ${guestsCount} ospiti`);
  ...
  if (config.bl && typeof config.bl === 'object') {
    ...
    if (hasAll) {
      ...
      // Fallback: somma da tutti i gruppi letto (escludendo 'all')
      debugLog('🛏️', "Sommando da gruppi letto individuali");
  ...
  debugLog('✅', `calculateOrderItems: ${items.length} items calcolati`);
