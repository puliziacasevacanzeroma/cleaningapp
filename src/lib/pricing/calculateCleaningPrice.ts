...
import type { Holiday, PriceBreakdownItem, PriceCalculationResult, PricingConfig } from "~/types/holiday";
import {
...
export function calculateEstimatedDuration(input: CalculateDurationInput): number {
  ...
  if (property) {
    if (serviceType.durationPerRoom && property.bedrooms > 1) {
      // @ts-expect-error TODO-FIX: TS18048 'duration' is possibly 'undefined'.
