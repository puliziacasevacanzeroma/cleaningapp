...
export async function getPropertiesByOwner(ownerId: string): Promise<Property[]> {
  // ⚡ OTTIMIZZATO: query filtrata per ownerId invece di scaricare tutto
 ...
 * - Pulizie (cleanings)
 * - Ordini biancheria (orders) - sia diretti che collegati a pulizie
 * - Ordini biancheria legacy (linen_orders) - se esistono
...
export async function deletePropertyWithCascade(propertyId: string): Promise<{
  ...
  // 2b. Elimina anche ordini collegati alle pulizie eliminate (per cleaningId)
  // Questo cattura ordini che potrebbero avere un propertyId diverso/mancante
  if (cleaningIds.length > 0) {
    // Firestore 'in' supporta max 30 valori, chunka se necessario
    for (let i = 0; i < cleaningIds.length; i += 30) {
      const chunk = cleaningIds.slice(i, i + 30);
      try {
        const linkedOrdersSnapshot = await getDocs(
          query(collection(db, "orders"), where("cleaningId", "in", chunk))
        );
        for (const docSnap of linkedOrdersSnapshot.docs) {
          // Evita di eliminare due volte lo stesso ordine
          try {
            await deleteDoc(doc(db, "orders", docSnap.id));
            deletedOrders++;
          } catch (e) {
            // Già eliminato nel passo precedente, ignora
          }
        }
      } catch (e) {
  ...
  // 2c. Elimina eventuali ordini nella collection linen_orders (legacy)
  try {
    const linenOrdersSnapshot = await getDocs(
      query(collection(db, "linen_orders"), where("propertyId", "==", propertyId))
    );
    for (const docSnap of linenOrdersSnapshot.docs) {
      await deleteDoc(doc(db, "linen_orders", docSnap.id));
      deletedOrders++;
    }
    if (linenOrdersSnapshot.docs.length > 0) {
    }
  } catch (e) {
  }
...
export async function getBookings(propertyId?: string): Promise<Booking[]> {
  ...
  let bookings = snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as Record<string, any>) } as Booking));
...
export async function getBookingById(id: string): Promise<Booking | null> {
  ...
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) return null;
  return { id: docSnap.id, ...(docSnap.data() as Record<string, any>) } as Booking;
}
...
export async function getInventory(category?: string): Promise<InventoryItem[]> {
  ...
  let items = snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as Record<string, any>) } as InventoryItem));
...
export interface Order {
  ...
  // Costo consegna (€10 per ordini standalone senza pulizia)
  deliveryFee?: number;          // Importo fee (default: 0 se con pulizia, 10 se standalone)
  deliveryFeeEnabled?: boolean;  // Admin può disattivare per singolo ordine
  createdAt: Timestamp;
  updatedAt: Timestamp;
}
