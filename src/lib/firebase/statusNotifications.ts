...
export async function createStatusChangeNotification(
  ...
  if (!notificationType) {
    if (process.env.NODE_ENV !== "production") console.log('Nessuna notifica necessaria per questo cambio stato');
    return [];
  }
      ...
      const notificationData = {
        ...
        link: data.entityType === 'CLEANING' 
          ? (recipient.role === 'ADMIN' 
            ...
            : recipient.role === 'OPERATORE_PULIZIE'
              ? `/operatore/pulizie/${data.entityId}`
              : `/proprietario/pulizie?id=${data.entityId}`)
          : (recipient.role === 'ADMIN'
