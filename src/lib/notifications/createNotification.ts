...
export async function createNotificationDirect(
  ...
  try {
    ...
    // Invia push notification (server-side)
    try {
      const { sendPushToUser, sendPushToRole } = await import("./sendPushNotification");
      ...
      if (params.relatedEntityId) pushData.relatedEntityId = params.relatedEntityId;
      if (params.link) pushData.link = params.link;

      if (params.recipientId) {
        await sendPushToUser(params.recipientId, content.title, content.message, pushData);
        ...
        await sendPushToRole(
          ...
          content.title, content.message, pushData
        );
      }
    } catch (pushError) {
      console.warn("⚠️ Errore push:", pushError);
    }
