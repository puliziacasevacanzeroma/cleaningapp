...
export default function CleaningActionModal({ 
  ...
  // 🔥 Stato per conferma spostamento pulizia in corso
  const [showInProgressConfirm, setShowInProgressConfirm] = useState(false);
  const [inProgressOperatorName, setInProgressOperatorName] = useState("");
  ...
  // Handle Move
  const handleMove = async (forceConfirm = false) => {
    if (!newDate) {
    ...
    try {
      const res = await fetch(`/api/cleanings/${cleaning.id}/move`, {
        ...
        body: JSON.stringify({
          ...
          confirmInProgress: forceConfirm, // 🔥 Flag conferma
        }),
      });

      const data = await res.json();

      // 🔥 Gestione conferma pulizia in corso
      if (res.status === 409 && data.requiresConfirmation) {
        setInProgressOperatorName(data.operatorName || "l'operatore");
  ...
  // 🔥 Conferma spostamento pulizia in corso
  const handleConfirmInProgressMove = async () => {
  ...
  return (
    ...
    >
      ...
      {/* 🔥 Dialog conferma spostamento pulizia in corso */}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-4">
                <strong>{inProgressOperatorName}</strong> ha già iniziato questa pulizia.
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleConfirmInProgressMove}
                  disabled={loading}
