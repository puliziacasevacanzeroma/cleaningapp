...
/**
 * Conta le lenzuola attualmente selezionate dall'utente
 */
const countCurrentLinenFromBl = (
  bl: Record<string, Record<string, number>> | undefined, 
  invLinen: LinenItem[]
): { matrimoniali: number; singole: number } => {
  let matrimoniali = 0;
  let singole = 0;

  if (!bl || !invLinen || invLinen.length === 0) return { matrimoniali, singole };
...
const validateLinenForBeds = (
  ...
  return {
    isValid: missingMatrimoniali === 0 && missingSingole === 0,
    missingMatrimoniali,
    missingSingole,
...
export default function EditCleaningModal({ isOpen, onClose, cleaning, property, onSuccess, userRole = "PROPRIETARIO" }: EditCleaningModalProps) {
  ...
  const linenValidation = useMemo((): LinenValidationResult => {
    ...
    return validateLinenForBeds(selectedBedsData, c.bl, invLinen);
  }, [linenEnabled, selectedBedsData, c.bl, invLinen]);
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-red-600 mt-1">
                                Per i letti selezionati servono almeno:
                              </p>
