...
export function CreaProprietaOwnerModal({ isOpen, onClose }: CreaProprietaOwnerModalProps) {
  ...
  const totalSteps = 3; // Solo 3 step per il proprietario
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = {
        ...
        // Niente prezzo - lo metterà l'admin
        // Niente proprietario - è l'utente corrente
      };
            ...
            <div className="space-y-4">
              ...
              <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-violet-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                  </svg>
                  <div>
                    <p className="text-sm font-semibold text-violet-800">Cosa succede dopo?</p>
                    <p className="text-xs text-violet-600 mt-1">La tua richiesta verrà inviata all'amministratore che definirà il prezzo della pulizia e attiverà la proprietà.</p>
                  </div>
                </div>
              </div>
            </div>
