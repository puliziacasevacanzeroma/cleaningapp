...
export function UtentiView() {
  ...
  const stats = {
    ...
    LAVANDERIA: users.filter(u => u.role === 'LAVANDERIA').length,
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      {showEditModal && userToAction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6" onClick={() => !isProcessing && setShowEditModal(false)}>
          <div className="absolute inset-0 bg-black/60" />
          <div className="relative bg-white rounded-[20px] w-full max-w-lg max-h-[82vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className={`bg-gradient-to-r ${roleConfig[userToAction.role]?.gradient || 'from-slate-500 to-slate-600'} px-4 pt-3 pb-3 flex-shrink-0`}>
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-[16px] font-bold text-white">Modifica Utente</h2>
                  <p className="text-white/70 text-[11px]">{userToAction.name}</p>
                </div>
                <button onClick={() => setShowEditModal(false)} className="w-7 h-7 rounded-lg bg-white/20 flex items-center justify-center active:scale-90 transition-all">
                  <svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            </div>
            
            <form onSubmit={handleEditUser} className="flex-1 overflow-y-auto p-4 space-y-3">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Nome completo</label>
                <input
                  type="text"
              ...
              <div className="flex gap-2 pt-2 pb-2">
                <button
                  ...
                  disabled={isProcessing}
                  className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-[12px] font-semibold active:scale-95 transition-all disabled:opacity-50"
                >
                  Annulla
                </button>
                <button
                  type="submit"
                  disabled={isProcessing}
                  className={`flex-1 py-2.5 rounded-xl text-[12px] font-semibold active:scale-95 transition-all disabled:opacity-50 bg-gradient-to-r ${roleConfig[userToAction.role]?.gradient || 'from-slate-500 to-slate-600'} text-white`}
                >
