  ...
  return (
    <div className="space-y-3">
      {services.map((service) => {
        ...
        const isTodayService = isSameDay(service.date, new Date());
        ...
        return (
          <div 
            ...
            className={`bg-white rounded-xl border overflow-hidden transition-all hover:shadow-lg hover:border-sky-200 ${
              isPast ? 'opacity-60' : ''
            } ${isTodayService ? 'border-amber-300 shadow-amber-100' : 'border-slate-200'}`}
          >
            {/* Header Card */}
            <div className="flex items-center gap-3 p-3">
              {/* Data */}
              <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center text-white flex-shrink-0 ${
                isPast ? 'bg-slate-400' : isTodayService ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 'bg-gradient-to-br from-sky-500 to-sky-600'
              }`}>
