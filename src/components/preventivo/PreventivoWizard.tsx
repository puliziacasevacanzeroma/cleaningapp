...
export function PreventivoWizard() {
  ...
  // v12: bolle + stelline animate nella sidebar (stessa resa dei banner del sito).
  // Generate una sola volta al mount; niente se l'utente ha "riduci movimento".
  useEffect(() => {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      b.className = "pv-bubble";
      const s = 6 + Math.random() * 22;
      b.style.width = `${s}px`;
      b.style.height = `${s}px`;
      b.style.left = `${Math.random() * 100}%`;
      const d = 12 + Math.random() * 11;
      b.style.animationDuration = `${d}s`;
      b.style.animationDelay = `${-Math.random() * d}s`;
      b.style.setProperty("--drift", `${Math.random() * 70 - 35}px`);
      box.appendChild(b);
    }
    for (let j = 0; j < 6; j++) {
      const sp = document.createElement("span");
      sp.className = "pv-spark";
      const sz = 8 + Math.random() * 10;
      sp.style.width = `${sz}px`;
      sp.style.height = `${sz}px`;
      sp.style.left = `${Math.random() * 90 + 4}%`;
      sp.style.top = `${Math.random() * 80 + 8}%`;
      const dd = 2.6 + Math.random() * 3;
      sp.style.animationDuration = `${dd}s`;
      sp.style.animationDelay = `${-Math.random() * dd}s`;
      sp.innerHTML = stella;
      box.appendChild(sp);
    }
  }, []);
  ...
  return (
    <div className="pv-widget">
      <aside className="pv-side">
        <span className="pv-halo1" aria-hidden="true" />
        <span className="pv-halo2" aria-hidden="true" />
        <div className="pv-fx" ref={fxRef} aria-hidden="true" />
