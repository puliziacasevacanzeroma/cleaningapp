...
import type { BillingFormData, BillingType, BillingValidationErrors } from "~/types/billing";
import {
  createEmptyBillingFormData,
  validateBillingForm,
  validateFiscalCodeMatch,
  isValidFiscalCode,
  formatFiscalCode,
  formatVatNumber,
  formatSdiCode,
      ...
      {formData.type === "persona_fisica" && (
        <div className={`bg-gray-50 rounded-xl p-4 ${sectionClasses}`}>
          ...
          <div className="grid gap-4 md:grid-cols-2 mb-4">
            <div>
              <label className={labelClasses}>Nome Intestatario *</label>
              <input
                type="text"
            ...
            <div>
              <label className={labelClasses}>Cognome Intestatario *</label>
              <input
                type="text"
          ...
          <div>
            ...
            <input
              ...
              maxLength={16}
              disabled={disabled}
              className={`w-full px-4 py-3 border rounded-xl transition-colors focus:ring-2 focus:ring-sky-500 focus:border-sky-500 uppercase ${disabled ? 'bg-gray-100 cursor-not-allowed' : ''} ${formData.fiscalCode.length === 16 ? (isValidFiscalCode(formData.fiscalCode) && formData.firstName.trim().length >= 2 && formData.lastName.trim().length >= 2 && validateFiscalCodeMatch(formData.firstName.trim(), formData.lastName.trim(), formData.fiscalCode) ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50') : shouldShowError("fiscalCode") ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
            />
            ...
            {formData.fiscalCode.length === 16 && formData.firstName.trim().length >= 2 && formData.lastName.trim().length >= 2 && (
              isValidFiscalCode(formData.fiscalCode) && validateFiscalCodeMatch(formData.firstName.trim(), formData.lastName.trim(), formData.fiscalCode)
                ? <p className="text-green-600 text-sm mt-1 flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Codice fiscale verificato</p>
                : <p className="text-red-600 text-sm mt-1 flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>{!isValidFiscalCode(formData.fiscalCode) ? 'Formato CF non valido' : 'Il CF non corrisponde al nome e cognome'}</p>
            )}
            {shouldShowError("fiscalCode") && formData.fiscalCode.length < 16 && (
