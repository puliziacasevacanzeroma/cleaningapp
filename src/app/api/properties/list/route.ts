...
export async function GET() {
  ...
  try {
    ...
    const q = adminDb.collection("properties").orderBy("name", "asc");
    const snapshot = await q.get();
    
    if (process.env.NODE_ENV !== "production") console.log(`⚡ API properties/list: ${snapshot.docs.length} docs in ${Date.now() - startTime}ms`);
