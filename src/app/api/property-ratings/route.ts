...
export async function GET(req: NextRequest) {
  try {
    ...
    if (cleaningId) {
      const snapshot = await adminDb.collection("propertyRatings")
    ...
    for (const rating of ratings) {
      // @ts-expect-error TODO-FIX: TS2339 Property 'scores' does not exist on type '{ createdAt: any; id: string; }'.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV !== "production") console.log(`⭐ Rating salvato: ${ratingRef.id} - Media: ${avgScore.toFixed(2)} - Issues: ${savedIssues.length}`);
