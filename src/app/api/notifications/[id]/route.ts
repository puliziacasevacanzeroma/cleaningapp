...
import { updateProperty } from "~/lib/firebase/firestore-data-admin";
import { getApiUser } from "~/lib/api-auth";
import { validateBody, NotificationActionSchema } from "~/lib/validation/schemas";
...
export async function PATCH(
  ...
  // ─────────────────────────────────────────────────────
    ...
    const { id } = await params;
    const body = await validateBody(request, NotificationActionSchema);
    ...
    switch (action) {
      ...
      case "reject": {
        ...
        // @ts-expect-error TODO-FIX: TS2554 Expected 2-3 arguments, but got 4.
