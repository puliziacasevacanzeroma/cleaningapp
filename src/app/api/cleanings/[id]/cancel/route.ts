  ...
  try {
    ...
    if (!reason || reason.trim().length < 3) {
      return NextResponse.json({ 
        error: "Inserisci un motivo per la cancellazione" 
      }, { status: 400 });
    }
    ...
    if (!isAdmin && !isOwner) {
      return NextResponse.json({ 
        error: "Non hai i permessi per cancellare questa pulizia" 
      }, { status: 403 });
    }
    
    // ─── VERIFICA STATO ───
    // Non si può cancellare una pulizia già completata o in corso (se non admin)
    if (cleaning.status === "COMPLETED" || cleaning.status === "completed") {
      return NextResponse.json({ 
        error: "Non puoi cancellare una pulizia già completata" 
      }, { status: 400 });
    }
    
    if (!isAdmin && (cleaning.status === "IN_PROGRESS" || cleaning.status === "in_progress")) {
      return NextResponse.json({ 
        error: "Non puoi cancellare una pulizia in corso" 
      }, { status: 400 });
    }
    
    const now = Timestamp.now();
    
    // ─── CREA RECORD CANCELLAZIONE (per evitare re-sync iCal) ───
    if (cleaning.bookingSource || cleaning.externalUid) {
      ...
      console.log(`🔐 Esclusione sync creata per pulizia cancellata`);
    }
    ...
    if (cleaning.laundryOrderId) {
      try {
        ...
        if (orderSnap.exists()) {
          ...
          // Cancella solo se non già in transito o completato
          if (order.status !== "IN_TRANSIT" && order.status !== "DELIVERED" && order.status !== "COMPLETED") {
            ...
            console.log(`📦 Ordine biancheria ${cleaning.laundryOrderId} cancellato`);
          }
        }
      } catch (orderError) {
        console.error("Errore cancellazione ordine:", orderError);
      }
    }
      ...
      if (operator.id && operator.id !== user.id) {
        try {
          ...
          await createNotification({
            ...
            message: `La pulizia di "${cleaning.propertyName}" del ${dateStr} è stata cancellata. Motivo: ${reason}`,
            type: "WARNING",
    ...
    // ─── CANCELLA O MARCA COME CANCELLATA ───
    if (deleteCompletely && isAdmin) {
      // Eliminazione completa (solo admin)
      await deleteDoc(cleaningRef);
      console.log(`🗑️ Pulizia ${id} eliminata completamente`);
    } else {
      // Marca come cancellata (mantiene storico)
      ...
      console.log(`❌ Pulizia ${id} marcata come cancellata`);
    }
    
    return NextResponse.json({ 
      success: true,
      deleted: deleteCompletely && isAdmin,
      laundryOrderCancelled,
      message: deleteCompletely && isAdmin 
        ? "Pulizia eliminata" 
        : "Pulizia cancellata"
    });
  } catch (error) {
