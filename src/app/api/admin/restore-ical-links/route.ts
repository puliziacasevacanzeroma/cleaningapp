  ...
  try {
    ...
    for (const backup of ICAL_LINKS_BACKUP) {
      ...
      if (!match) {
        ...
        if (process.env.NODE_ENV !== "production") console.log(`❌ Non trovata: ${backup.name}`);
        continue;
      }
      ...
      try {
        ...
        await adminDb.collection('properties').doc(match.id).update(updateData);
    ...
    if (process.env.NODE_ENV !== "production") console.log(`   ✅ Aggiornate: ${results.updated.length}`);
    if (process.env.NODE_ENV !== "production") console.log(`   ❌ Non trovate: ${results.notFound.length}`);
    if (process.env.NODE_ENV !== "production") console.log(`   ⚠️ Errori: ${results.errors.length}`);
