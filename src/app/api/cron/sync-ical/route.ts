                ...
                if (!ci || !co || !isSameDay(ci, effStart) || !isSameDay(co, e.dtend) || !existing.icalUid || existing.icalUid !== e.uid) {
                      ...
                      if (!alertDocF.exists) {
                        ...
                        for (const adminDoc of adminsSnapF.docs) {
                          await adminDb.collection('notifications').add({
                            title: '🚪 Giorni liberati su una prenotazione',
                            message: `🏠 ${prop.name}\n\nUna prenotazione che iniziava il ${fmtItF(oldCiStr)} ora nel calendario risulta libera fino al ${fmtItF(newStartStr)}: i primi giorni si sono liberati.\n\nDi solito succede per una partenza anticipata o una cancellazione parziale. Non serve fare niente sul gestionale, la pulizia di fine soggiorno resta al suo posto.\n\n👉 Ti avvisiamo solo perché quei giorni ora sono liberi: se vuoi, puoi rimetterli in vendita su Booking.`,
                        ...
                        try {
                          ...
                          await sendPushNotification(
                            { title: '🚪 Giorni liberati su Booking', body: `${prop.name}: una prenotazione ora parte il ${fmtItF(newStartStr)} (era ${fmtItF(oldCiStr)}), i primi giorni si sono liberati. Puoi rimetterli in vendita.` },
                    ...
                    try {
                      ...
                      if (!alertDocL.exists) {
                        ...
                        for (const adminDoc of adminsSnapL.docs) {
                          await adminDb.collection('notifications').add({
                            title: '⚠️ Possibile pulizia mancante — una prenotazione inizia prima',
                            message: `🏠 ${prop.name}\n\nUna prenotazione ora inizia prima del previsto: partiva il ${fmtItL(oldCiStr)}, adesso il calendario la fa partire dal ${fmtItL(newCiStr)}.\n\nQuesto di solito vuol dire che è arrivato un nuovo ospite prima, attaccato a quello che c'era già: Booking li unisce e non ci dice dove finisce uno e inizia l'altro. In questo caso il cambio ospiti sarebbe il ${fmtItL(oldCiStr)} — ma una pulizia in quel giorno non è stata creata, perché da qui non possiamo sapere con certezza se serve.\n\n👉 Controlla su Booking le prenotazioni intorno al ${fmtItL(oldCiStr)}:\n• Se il ${fmtItL(oldCiStr)} entra un nuovo ospite → crea tu la pulizia quel giorno, altrimenti chi arriva trova la casa sporca.\n• Se è lo stesso ospite che è semplicemente entrato prima → è tutto a posto, non fare niente.`,
                            type: 'WARNING', recipientRole: 'ADMIN', recipientId: adminDoc.id,
                        ...
                        try {
                          const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                          await sendPushNotification(
                            { title: '⚠️ Possibile pulizia MANCANTE', body: `${prop.name}: una prenotazione ora inizia prima — possibile cambio ospiti il ${fmtItL(oldCiStr)} senza pulizia. Controlla su Booking e creala se serve.` },
                            { role: 'ADMIN', priority: 'high' }
                          );
                        } catch {}
                        // 📣 Notifica anche al PROPRIETARIO (con spiegazione del perché)
                        try {
                          const ownerIdN = typeof prop.ownerId === 'string' ? prop.ownerId.trim() : '';
                          if (ownerIdN && ownerIdN !== 'pending') {
                            await adminDb.collection('notifications').add({
                              title: '⚠️ Una prenotazione inizia prima — controlla su Booking',
                              message: `🏠 ${prop.name}\n\nUna tua prenotazione ora inizia prima: partiva il ${fmtItL(oldCiStr)}, adesso parte dal ${fmtItL(newCiStr)}.\n\nSe davanti si è aggiunto un nuovo ospite, il ${fmtItL(oldCiStr)} ci sarebbe un cambio che ha bisogno di una pulizia — che al momento non è prevista.\n\n👉 Apri Booking e controlla intorno al ${fmtItL(oldCiStr)}: se sono due ospiti diversi, scrivici così programmiamo la pulizia del cambio. Se è lo stesso ospite entrato prima, va bene così.`,
                            ...
                            try {
                              const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                              await sendPushNotification(
                                { title: '⚠️ Verifica su Booking', body: `${prop.name}: una prenotazione inizia prima del previsto — possibile cambio ospiti il ${fmtItL(oldCiStr)}. Controlla su Booking e avvisaci.` },
                  ...
                  if (co && !isSameDay(co, e.dtend) && isBookingRightExtension) {
                    ...
                    for (const cDoc of recCleaningsSnap.docs) {
                      ...
                      if (!keptCleaningId) keptCleaningId = cDoc.id;
                      console.log(`[TURNOVER-RECOVERY] Pulizia ${cDoc.id} mantenuta al ${oldCoStr} (blocco esteso a ${newCoStr})`);
                    }

                    // 1b. Se al vecchio checkout non esiste NESSUNA pulizia (era stata
                    // spostata o cancellata), il turnover resterebbe scoperto: cerca per
                    // data e, se manca davvero, CREALA (lockedFromSync: non va rispostata).
                    if (!keptCleaningId) {
                      ...
                      if (atOldCo) {
                      ...
                      } else if (!excludedDates.has(oldCoStr)) {
                        ...
                        if (!qO.empty) {
                          ...
                          const guestsCountO = existing.guests || prop.maxGuests || 2;
                          const cpO = prop.cleaningPrice || 0;
                          const holO = getHolidayFee(co, cpO, holidays);
                          const cleaningRefO = await adminDb.collection('cleanings').add({
                            propertyId: prop.id, propertyName: prop.name, propertyAddress: prop.address || '',
                            scheduledDate: Timestamp.fromDate(co), scheduledTime: prop.checkOutTime || '10:00',
                            status: 'SCHEDULED', bookingSource: source, bookingId: existing.id,
                            guestsCount: guestsCountO, guestName: existing.guestName || getGuestName(e, source),
                            price: cpO, contractPrice: cpO, serviceType: 'STANDARD', serviceTypeName: 'Pulizia Standard',
                          ...
                          stats.cleanings++;
                          cleanings.push({ id: cleaningRefO.id, scheduledDate: Timestamp.fromDate(co), status: 'SCHEDULED', bookingId: existing.id } as any);
                          keptCleaningId = cleaningRefO.id;
                          auditLog.cleaningCreated({ cleaningId: cleaningRefO.id, propertyId: prop.id, propertyName: prop.name, source: 'cron/sync-ical:TURNOVER-RECOVERY-OLDCO', scheduledDate: oldCoStr, bookingId: existing.id, guestsCount: guestsCountO, guestName: existing.guestName });
                          if (!prop.usesOwnLinen) {
                            const existingOrderO = ordersByDateStr.get(oldCoStr) || ordersByCleaningId.get(cleaningRefO.id);
                            if (!existingOrderO) {
                              const linenItemsO = calculateLinenItemsForProperty(prop, guestsCountO);
                              if (linenItemsO.length > 0) {
                                const orderIdO = await createLinenOrder(cleaningRefO.id, prop, co, linenItemsO);
                                if (orderIdO) {
                                  stats.linenOrders++;
                                  ordersByCleaningId.set(cleaningRefO.id, { id: orderIdO });
                                  ordersByDateStr.set(oldCoStr, { id: orderIdO });
                                  await adminDb.collection('cleanings').doc(cleaningRefO.id).update({ laundryOrderId: orderIdO, requiresLaundry: true });
                                }
                              }
                            }
                          }
                    ...
                    try {
                      ...
                      if (!alertDoc.exists) {
                        ...
                        // Metadati azione: permettono al client di aprire il modal
                        // "Lascia pulizia / Cancella pulizia" (API: /api/turnover-decision)
                        const turnoverActionFields = keptCleaningId ? {
                          actionRequired: true,
                          // ⚠️ actionStatus è OBBLIGATORIO: tutti i filtri "azioni
                          // richieste" (NotificheAdminContent, useNotifications)
                          // cercano actionRequired && actionStatus === 'PENDING'.
                          // Senza, la notifica non risultava mai "da gestire".
                        ...
                        for (const adminDoc of adminsSnapR.docs) {
                          await adminDb.collection('notifications').add({
                            title: '🔁 Una prenotazione si è allungata — verifica il cambio ospiti',
                        ...
                        try {
                          const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                          await sendPushNotification(
                            { title: '🔁 Pulizia turnover recuperata', body: `${prop.name}: una prenotazione si è allungata (prima finiva il ${fmtIt(oldCoStr)}, ora il ${fmtIt(newCoStr)}). Abbiamo tenuto la pulizia del ${fmtIt(oldCoStr)}. Se è lo stesso ospite che resta, cancellala.` },
                            { role: 'ADMIN', priority: 'high' }
                          );
                        } catch {}
                        // 📣 Notifica anche al PROPRIETARIO (con spiegazione del perché)
                        try {
                          ...
                          ...turnoverActionFields,
                            ...
                            try {
                              const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                              await sendPushNotification(
                                { title: '🔁 Verifica cambio ospiti', body: `${prop.name}: una prenotazione si è allungata — abbiamo previsto una pulizia il ${fmtIt(oldCoStr)}. Controlla su Booking se è un nuovo ospite o lo stesso che resta.` },
                                { userId: ownerIdN, priority: 'high' }
                              );
                            } catch {}
                          }
                    ...
                    for (const cDoc of cleaningsForBookingSnap.docs) {
                      ...
                      if (cData.status === 'ASSIGNED' && cData.assignedTo) {
                        try {
                          await adminDb.collection('notifications').add({
                            title: '📅 La pulizia è stata spostata di data',
                            message: `La pulizia di ${prop.name} è stata spostata al ${e.dtend.toLocaleDateString('it-IT')}.\n\nMotivo: l'ospite (${existing.guestName || 'ospite'}) ha cambiato le date della prenotazione. Controlla la tua lista aggiornata.`,
                            type: 'INFO',
                    ...
                    try {
                      ...
                      if (!alertDocN.exists) {
                        ...
                        for (const adminDoc of adminsSnapN.docs) {
                          await adminDb.collection('notifications').add({
                            title: `⚠️ Prenotazione lunga: controlla se ci sono più ospiti`,
                            message: `🏠 ${prop.name}\n\nÈ arrivata una prenotazione di ${nightsNew} notti, dal ${fmtItN(ciStrN)} al ${fmtItN(coStrN)}.\n\nA volte Booking mette insieme più prenotazioni una dietro l'altra e le mostra come se fossero una sola. Se in questo periodo si sono alternati ospiti diversi, in mezzo serve una pulizia — ma noi da qui non riusciamo a vederlo.\n\nAbbiamo previsto la pulizia solo alla fine, il ${fmtItN(coStrN)}.\n\n👉 Cosa fare: apri Booking e guarda le prenotazioni tra il ${fmtItN(ciStrN)} e il ${fmtItN(coStrN)}.\n• Se è un unico ospite per tutto il periodo → è tutto a posto, non fare niente.\n• Se sono più ospiti che si alternano → aggiungi tu la pulizia nel giorno di ogni cambio, altrimenti chi arriva trova la casa sporca.`,
                            type: 'WARNING', recipientRole: 'ADMIN', recipientId: adminDoc.id,
                            senderId: 'system', senderName: 'Sync iCal - Turnover Recovery',
                            status: 'UNREAD', createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
                          });
                        ...
                        try {
                          const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                          await sendPushNotification(
                            { title: '⚠️ Possibili pulizie mancanti', body: `${prop.name}: prenotazione lunga di ${nightsNew} notti (${fmtItN(ciStrN)}→${fmtItN(coStrN)}). Se dentro ci sono più ospiti, controlla su Booking e aggiungi le pulizie dei cambi.` },
                        ...
                        try {
                          ...
                          if (ownerIdN && ownerIdN !== 'pending') {
                            await adminDb.collection('notifications').add({
                              title: `⚠️ Prenotazione lunga da verificare su Booking`,
                              message: `🏠 ${prop.name}\n\nÈ arrivata una prenotazione di ${nightsNew} notti, dal ${fmtItN(ciStrN)} al ${fmtItN(coStrN)}.\n\nA volte Booking unisce più prenotazioni vicine e le mostra come una sola. Se in questo periodo ci sono ospiti diversi, tra un soggiorno e l'altro serve una pulizia che al momento non è prevista.\n\n👉 Ti chiediamo 30 secondi: apri Booking e controlla quelle date. Se sono più soggiorni diversi, scrivici le date dei cambi e programmiamo noi le pulizie. Se è un'unica prenotazione lunga, va bene così.`,
                              type: 'WARNING', recipientRole: 'PROPRIETARIO', recipientId: ownerIdN,
                              senderId: 'system', senderName: 'Sync iCal - Turnover Recovery',
                              status: 'UNREAD', createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
                            });
                            try {
                              const { sendPushNotification } = await import('~/lib/notifications/sendPushNotification');
                              await sendPushNotification(
                                { title: '⚠️ Verifica prenotazioni su Booking', body: `${prop.name}: prenotazione lunga di ${nightsNew} notti in arrivo. Se contiene più soggiorni diversi, dicci le date dei cambi.` },
                                { userId: ownerIdN, priority: 'high' }
                              );
                            } catch {}
                          }
        ...
        for (let i = 0; i < bookings.length - 1; i++) {
          for (let j = i + 1; j < bookings.length; j++) {
            ...
            if (b.checkIn < a.checkOut && a.source !== b.source) {
              ...
              const msg = `🏠 ${propName}\n\nDue prenotazioni da canali diversi occupano la stessa casa nelle stesse date:\n\n` +
                `1️⃣ ${a.source.toUpperCase()} — "${a.guestName}": dal ${fmtDate(a.checkIn)} al ${fmtDate(a.checkOut)}\n` +
                `2️⃣ ${b.source.toUpperCase()} — "${b.guestName}": dal ${fmtDate(b.checkIn)} al ${fmtDate(b.checkOut)}\n\n` +
                `Potrebbe essere un overbooking (due ospiti diversi prenotati insieme) oppure la stessa prenotazione arrivata da due canali sincronizzati.\n\n👉 Controlla subito su ${a.source.toUpperCase()} e ${b.source.toUpperCase()}: se sono davvero due ospiti diversi, uno va spostato o cancellato prima che si presentino entrambi.`;
              overlaps.push(msg);
            }
          }
        }
        ...
        for (const overlapMsg of overlaps.slice(0, 10)) {
          ...
          if (!existingDoc.exists) {
            ...
            for (const adminDoc of adminsSnap.docs) {
              await adminDb.collection('notifications').add({
                title: `⚠️ Attenzione: due prenotazioni si sovrappongono`,
                message: overlapMsg,
                type: 'WARNING',
                recipientRole: 'ADMIN',
