...
export function DashboardMobileClient({ userName }: DashboardMobileClientProps) {
  ...
  const confirmTime = async () => {
    ...
    try {
      ...
      setCleanings(prev => prev.map(c => c.id === currentCardId ? { ...c, scheduledTime: timeStr } : c));
  ...
  const selectOperator = async (operator: Operator) => {
    ...
    try {
      ...
      setCleanings(prev => prev.map(c => c.id === currentCardId ? { ...c, operator: { id: operator.id, name: operator.name || "" }, status: "assigned" } : c));
  ...
  const getStatus = (c: Cleaning) => c.status === "completed" ? "done" : c.status === "in_progress" ? "inprogress" : "todo";
  ...
  const stats = { todo: cleanings.filter(c => getStatus(c) === "todo").length, inprogress: cleanings.filter(c => getStatus(c) === "inprogress").length, done: cleanings.filter(c => getStatus(c) === "done").length };
  const totalEarnings = cleanings.length * 85;
  const getInitials = (name: string | null) => name ? name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2) : "?";
  const getShortName = (name: string | null) => { if (!name) return ""; const p = name.split(" "); return p.length >= 2 ? `${p[0]} ${p[1]?.charAt(0)}.` : p[0] || ""; };
    ...
    <div className="min-h-screen bg-slate-100 pb-24">
      <style jsx global>{`
        * { font-family: 'Inter', -apple-system, sans-serif; }
        ...
        .success-toast { position: fixed; top: 80px; left: 50%; transform: translateX(-50%) translateY(-20px) scale(0.9); opacity: 0; visibility: hidden; transition: all 0.4s ease; z-index: 200; }
        ...
        .date-btn { background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); }
        .filter-btn.active { background: rgba(255,255,255,0.35); transform: scale(1.02); }
        .time-scroll { height: 180px; overflow-y: auto; scroll-snap-type: y mandatory; mask-image: linear-gradient(to bottom, transparent, black 25%, black 75%, transparent); }
        .time-scroll::-webkit-scrollbar { display: none; }
        .time-item { height: 60px; scroll-snap-align: center; }
      ...
      <main className="px-4 py-4">
        <div className="hero-gradient rounded-3xl p-4 mb-4 shadow-xl shadow-indigo-500/20">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="inline-flex items-center gap-1 bg-white/20 rounded-full px-2.5 py-1">
              <svg className="w-3.5 h-3.5 text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-slate-800">{activeFilter === "todo" ? "Da fare" : activeFilter === "inprogress" ? "In corso" : activeFilter === "done" ? "Completate" : "Tutte le pulizie"}</h2>
          <span className="text-xs text-slate-400">{filteredCleanings.length} attività</span>
        </div>

        <div className="space-y-3">
          {loading ? [...Array(3)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-4 animate-pulse"><div className="flex gap-3"><div className="w-28 h-32 bg-slate-200 rounded-xl"></div><div className="flex-1"><div className="h-4 bg-slate-200 rounded w-3/4 mb-2"></div><div className="h-3 bg-slate-200 rounded w-1/2"></div></div></div></div>
          )) : filteredCleanings.length === 0 ? (
            <div className="text-center py-12"><p className="text-slate-500">Nessuna pulizia per questa data</p></div>
          ) : filteredCleanings.map((cleaning, index) => {
            const status = getStatus(cleaning);
            const isDone = status === "done";
            const isInProgress = status === "inprogress";
            return (
              <div key={cleaning.id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm ${isDone ? "border-emerald-200 opacity-70" : isInProgress ? "border-2 border-sky-300" : "border-slate-200"}`}>
                <div className="flex items-center">
                  <div className="w-28 h-32 flex-shrink-0 relative">
                    <img src={getPropertyImage(index)} className="w-full h-full object-cover" alt=""/>
                    {isDone && <div className="absolute inset-0 bg-emerald-500/20"></div>}
                    <div className={`absolute top-2 left-2 px-2 py-1 text-white text-[10px] font-bold rounded-lg flex items-center gap-1 ${isDone ? "bg-emerald-500" : isInProgress ? "bg-sky-500" : "bg-amber-500"}`}>
                  ...
                  <div className="flex-1 p-3">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      {isDone ? (
                        <div className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full"><span className="text-sm font-semibold">{cleaning.scheduledTime || "10:00"}</span></div>
                      ) : (
                        <button onClick={() => openTimePicker(cleaning)} className="flex items-center gap-1.5 text-sky-600 bg-sky-50 border border-sky-100 px-3 py-1.5 rounded-full">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                          <span className="text-sm font-semibold">{cleaning.scheduledTime || "10:00"}</span>
                        </button>
                      )}
                      {isDone ? (
                        <div className="flex items-center gap-1.5 text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full"><span className="text-sm font-semibold">{cleaning.booking?.guestsCount || 2}</span></div>
                      ) : (
                        <button onClick={() => openGuestsPicker(cleaning)} className="flex items-center gap-1.5 text-violet-600 bg-violet-50 border border-violet-100 px-3 py-1.5 rounded-full">
                    ...
                    <div className="flex items-center gap-2">
                      {cleaning.operator && (
                        <button onClick={() => !isDone && openOperatorPicker(cleaning)} className={`flex items-center gap-1 text-white pl-2 pr-1.5 py-1 rounded-full ${isDone ? "bg-slate-400" : "bg-emerald-500"}`}>
                          ...
                          {!isDone && <svg className="w-4 h-4 ml-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>}
                        </button>
                      )}
                      {!isDone && !isInProgress && (
                        <button onClick={() => openOperatorPicker(cleaning)} className="w-8 h-8 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"/></svg>
                        </button>
                      )}
                    </div>
                  </div>
                  <div className="pr-3"><svg className="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"/></svg></div>
                </div>
              </div>
            );
          })}
      ...
      {showOverlay && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50" onClick={closeAll}></div>}
      ...
      <div className={`picker-modal shadow-2xl ${showTimePicker ? "active" : ""}`}>
        <div className="p-6 pb-8">
          ...
          <div className="text-center mb-8"><span className="text-6xl font-extrabold text-slate-800">{selectedHour.toString().padStart(2, "0")}:{selectedMin.toString().padStart(2, "0")}</span></div>
          <div className="flex items-center justify-center gap-2 mb-8">
            <div className="relative w-24">
              ...
              <div className="time-scroll">
                ...
                {[...Array(18)].map((_, i) => (<div key={i} className={`time-item flex items-center justify-center cursor-pointer ${selectedHour === i + 6 ? "text-3xl font-bold text-slate-900" : "text-xl text-slate-300"}`} onClick={() => setSelectedHour(i + 6)}>{(i + 6).toString().padStart(2, "0")}</div>))}
                <div className="h-[60px]"></div>
              </div>
            </div>
            ...
            <div className="relative w-24">
              ...
              <div className="time-scroll">
                <div className="h-[60px]"></div>
                {[0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55].map((min) => (<div key={min} className={`time-item flex items-center justify-center cursor-pointer ${selectedMin === min ? "text-3xl font-bold text-slate-900" : "text-xl text-slate-300"}`} onClick={() => setSelectedMin(min)}>{min.toString().padStart(2, "0")}</div>))}
                <div className="h-[60px]"></div>
              </div>
            </div>
          </div>
          <button onClick={confirmTime} className="w-full py-4 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-2xl font-bold text-lg shadow-lg">Conferma</button>
        </div>
      </div>

      <div className={`picker-modal shadow-2xl ${showOperatorPicker ? "active" : ""}`} style={{maxHeight: "50vh"}}>
        <div className="p-5 pb-6">
          ...
          <div className="relative mb-4">
            ...
            <input type="text" placeholder="Cerca operatore..." value={operatorSearch} onChange={(e) => setOperatorSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-slate-100 rounded-xl text-sm outline-none focus:ring-2 focus:ring-sky-500"/>
          </div>
          <div className="overflow-y-auto space-y-2" style={{maxHeight: "calc(50vh - 160px)"}}>
            {filteredOperators.length === 0 ? <p className="text-center text-slate-400 py-8">Nessun operatore trovato</p> : filteredOperators.map((op, i) => {
              const colors = ["from-blue-400 to-blue-600", "from-pink-400 to-rose-600", "from-violet-400 to-purple-600", "from-emerald-400 to-teal-600", "from-amber-400 to-orange-600"];
              return (<button key={op.id} onClick={() => selectOperator(op)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-50 active:bg-slate-100">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${colors[i % colors.length]} flex items-center justify-center text-white font-bold`}>{op.name?.charAt(0) || "?"}</div>
                <div className="text-left flex-1"><p className="font-semibold text-slate-800">{op.name}</p><p className="text-xs text-slate-400">{op.email}</p></div>
      ...
      <div className={`picker-modal shadow-2xl ${showGuestsPicker ? "active" : ""}`}>
        <div className="p-5 pb-6">
          ...
          <div className="flex items-center justify-between py-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center"><svg className="w-5 h-5 text-indigo-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg></div>
              <p className="font-semibold text-slate-800">Adulti</p>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={() => setAdultsCount(Math.max(1, adultsCount - 1))} disabled={adultsCount <= 1} className="w-10 h-10 rounded-full border-2 border-slate-200 flex items-center justify-center text-slate-400 disabled:opacity-30"><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M20 12H4"/></svg></button>
              <span className="text-xl font-bold text-slate-800 w-8 text-center">{adultsCount}</span>
              <button onClick={() => setAdultsCount(Math.min(10, adultsCount + 1))} className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-200 to-indigo-400 flex items-center justify-center text-white"><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M12 4v16m8-8H4"/></svg></button>
            </div>
          </div>
          <div className="flex items-center justify-between py-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-rose-50 flex items-center justify-center"><svg className="w-5 h-5 text-rose-300" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="6" r="3"/><path d="M12 11c-2 0-4 1.5-4 3v4h8v-4c0-1.5-2-3-4-3z"/></svg></div>
              <div><p className="font-semibold text-slate-800">Neonati</p><p className="text-xs text-slate-400">0-2 anni</p></div>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={() => setInfantsCount(Math.max(0, infantsCount - 1))} disabled={infantsCount <= 0} className="w-10 h-10 rounded-full border-2 border-slate-200 flex items-center justify-center text-slate-400 disabled:opacity-30"><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M20 12H4"/></svg></button>
              <span className="text-xl font-bold text-slate-800 w-8 text-center">{infantsCount}</span>
              <button onClick={() => setInfantsCount(Math.min(5, infantsCount + 1))} className="w-10 h-10 rounded-full bg-gradient-to-br from-rose-200 to-rose-400 flex items-center justify-center text-white"><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M12 4v16m8-8H4"/></svg></button>
            </div>
          </div>
          <div className="flex items-end justify-center gap-1 h-16 mb-4">
            {[...Array(adultsCount)].map((_, i) => (<div key={`a${i}`} className="flex flex-col items-center"><div className="w-5 h-5 rounded-full bg-indigo-200"></div><div className="w-7 h-9 bg-indigo-300 rounded-t-xl rounded-b-lg mt-0.5"></div></div>))}
            {[...Array(infantsCount)].map((_, i) => (<div key={`i${i}`} className="flex flex-col items-center"><div className="w-4 h-4 rounded-full bg-rose-200"></div><div className="w-5 h-6 bg-rose-300 rounded-t-lg rounded-b-md mt-0.5"></div></div>))}
          </div>
          <p className="text-center text-sm text-slate-500 mb-4">{infantsCount > 0 ? `${adultsCount} adulti + ${infantsCount} neonati` : `${adultsCount + infantsCount} ospiti`}</p>
          <button onClick={confirmGuests} className="w-full py-4 bg-gradient-to-r from-violet-500 to-purple-600 text-white rounded-2xl font-bold text-lg shadow-lg">Conferma</button>
        </div>
      </div>
    </div>
  );
