  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-100 px-4 pt-4 pb-3 sticky top-0 z-20">
        <h1 className="text-[20px] font-bold text-slate-800 mb-3">Centro Messaggi</h1>
        <div className="relative bg-slate-100 rounded-[14px] p-1">
          <div 
            className={`absolute top-1 left-1 w-[calc(50%-4px)] h-[calc(100%-8px)] bg-gradient-to-r from-sky-500 to-blue-500 rounded-[10px] shadow-[0_2px_8px_rgba(14,165,233,.25)] transition-transform duration-300 ease-[cubic-bezier(.4,0,.2,1)] ${tab === "segnalazioni" ? "translate-x-full" : ""}`} 
          />
          {(["notifiche", "segnalazioni"] as Tab[]).map(t => (
            ...
            >
              <Ic d={t === "notifiche" ? ic.bell : ic.warn} className="w-4 h-4" />
      ...
      <div className="animate-[fadeUp_.3s_ease]" key={tab}>
        {tab === "notifiche" ? <NotificheAdminContent /> : <SegnalazioniAdminContent />}
      </div>
