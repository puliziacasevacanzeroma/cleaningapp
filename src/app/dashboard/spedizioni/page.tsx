...
export default function SpedizioniPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      {/* Banner slim */}
      <div className="relative overflow-hidden rounded-2xl mb-5 px-5 py-4 shadow-sm" style={{ background: "linear-gradient(135deg,#2563eb,#6366f1 55%,#7c3aed)" }}>
        <div className="relative flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center flex-shrink-0">
              <span className="w-5 h-5 text-white">{I.box}</span>
            </div>
            <div className="min-w-0">
              <h1 className="text-lg font-bold text-white leading-tight">Spedizioni Prodotti</h1>
              <p className="text-white/70 text-xs">{shipments.length} {shipments.length === 1 ? "spedizione" : "spedizioni"} · consegne prodotti pulizia</p>
            </div>
          </div>
          <button onClick={() => setShowNew(true)} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white text-indigo-700 font-semibold text-sm shadow-sm hover:bg-white/90 transition flex-shrink-0">
