...
export default function AdminStatistichePage() {
  ...
  useEffect(() => {
    const unsubs: (() => void)[] = [];
    unsubs.push(onSnapshot(query(collection(db, "cleanings"), orderBy("scheduledDate", "desc")), s => setCleanings(s.docs.map(d => ({ id: d.id, ...(d.data() as Record<string, any>) })) as Cleaning[])));
    unsubs.push(onSnapshot(query(collection(db, "orders"), orderBy("createdAt", "desc")), s => setOrders(s.docs.map(d => ({ id: d.id, ...(d.data() as Record<string, any>) })) as Order[])));
    unsubs.push(onSnapshot(collection(db, "properties"), s => setProperties(s.docs.map(d => ({ id: d.id, ...(d.data() as Record<string, any>) })) as Property[])));
    unsubs.push(onSnapshot(collection(db, "users"), s => { setUsers(s.docs.map(d => ({ id: d.id, ...(d.data() as Record<string, any>) })) as User[]); setLoading(false); }));
