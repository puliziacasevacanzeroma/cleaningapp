...
// 🚀 PERF v2: Cache localStorage per l'operatore, stesso pattern del DashboardClientWrapper admin.
// Scopo: al secondo ingresso (mattino dopo) le card delle pulizie vengono mostrate SUBITO dalla
// cache, lo spinner "Caricamento pulizie..." sparisce. Il listener onSnapshot continua a partire
// in parallelo e sovrascrive la cache appena arriva il primo snapshot — i dati freschi vincono
// sempre sui dati cached. La cache è per-utente (chiave include userId) così in dispositivi
// condivisi o cambi utente non si mostrano i dati dell'altro.
//
// Serializzazione Timestamp: il solo campo Timestamp letto nel JSX è `completedAt` via
// `cl.completedAt?.toDate?.()`. Prima di salvare in cache lo convertiamo a stringa ISO;
// in lettura ricostruiamo uno shim `{ toDate: () => new Date(iso) }` così il JSX esistente
// funziona senza modifiche. `propertyImages` viene cachato separatamente per evitare lo
// sfarfallio delle foto tra cache load e getDoc completion.

const CACHE_KEY_CLEANINGS = (userId: string) => `operatore_cleanings_cache_${userId}`;
const CACHE_KEY_IMAGES = (userId: string) => `operatore_property_images_cache_${userId}`;

// Chiave giorno corrente (YYYY-MM-DD) per invalidare la cache al cambio di giorno:
// se la cache è stata scritta ieri, non mostriamo dati di ieri al posto dello spinner.
function todayKey(): string {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

function loadCachedCleanings(userId: string): any[] | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(CACHE_KEY_CLEANINGS(userId));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.items)) return null;
    // Invalida cache se è di un giorno diverso — niente flash di dati di ieri
    if (parsed.dayKey !== todayKey()) return null;
    // Ricostruisci shim toDate() su completedAt per compatibilità col JSX esistente
    return parsed.items.map((c: any) => ({
      ...c,
      completedAt: c.completedAt ? { toDate: () => new Date(c.completedAt) } : undefined,
    }));
  } catch { return null; }
}

function saveCachedCleanings(userId: string, cleanings: any[]): void {
  if (typeof window === "undefined") return;
  try {
    // Serializza Timestamp → ISO string per poter usare JSON.stringify
    const items = cleanings.map((c: any) => ({
      ...c,
      completedAt: c.completedAt?.toDate?.()?.toISOString() || null,
      // Altri campi Timestamp potenzialmente presenti ma non usati nel JSX — li strippiamo
      // per evitare oggetti non-serializzabili che possono far fallire JSON.stringify
      scheduledDate: undefined,
      startedAt: undefined,
      createdAt: undefined,
      updatedAt: undefined,
    }));
    localStorage.setItem(CACHE_KEY_CLEANINGS(userId), JSON.stringify({ dayKey: todayKey(), items }));
  } catch {}
}

function loadCachedImages(userId: string): Record<string, string> {
  if (typeof window === "undefined") return {};
  try {
    const raw = localStorage.getItem(CACHE_KEY_IMAGES(userId));
    if (!raw) return {};
    const parsed = JSON.parse(raw);
...
function saveCachedImages(userId: string, images: Record<string, string>): void {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(CACHE_KEY_IMAGES(userId), JSON.stringify(images));
  } catch {}
}

export default function OperatoreDashboard() {
  const { user } = useAuth();
  // 🔄 Inizializza da cache: se c'è, le card compaiono SUBITO senza spinner.
  const [cleanings, setCleanings] = useState<any[]>(() => loadCachedCleanings(user?.id || "") || []);
  const [loading, setLoading] = useState(() => loadCachedCleanings(user?.id || "") === null);
  const [homeTab, setHomeTab] = useState<HomeTab>("oggi");
  const [propertyImages, setPropertyImages] = useState<Record<string, string>>(() => loadCachedImages(user?.id || ""));

  useEffect(() => {
    if (!user) return;

    // 🔄 Se utente diverso da quando la cache è stata popolata (caso raro ma possibile
    // con login/logout nello stesso tab), ricarica dalla cache del nuovo utente
    const cached = loadCachedCleanings(user.id);
    if (cached) {
      setCleanings(cached);
      setLoading(false);
    }
    const cachedImages = loadCachedImages(user.id);
    if (Object.keys(cachedImages).length > 0) {
      setPropertyImages(cachedImages);
    }
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      setLoading(false);
      // 💾 Aggiorna cache con i dati freschi
      saveCachedCleanings(user.id, filtered);
        ...
        try {
          ...
          if (snap.exists() && snap.data().imageUrl) {
            setPropertyImages(prev => {
              const next = { ...prev, [pid]: snap.data()!.imageUrl };
              // 💾 Aggiorna cache immagini
              saveCachedImages(user.id, next);
              return next;
            });
          }
        } catch {}
