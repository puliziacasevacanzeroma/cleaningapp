...
interface PropertyData {
  ...
  // Configurazione letti singoli (usato da EditCleaningModal e CleaningWizard)
  bedsConfig?: Array<{
    id: string;
    type?: string;
    tipo?: string;
...
export default function CleaningDetailPage() {
  ...
  // Cache della property: la carichiamo una sola volta, non ad ogni snapshot della cleaning
  const propertyCache = useRef<PropertyData | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubCleaning = onSnapshot(
      ...
      async (cleaningSnap) => {
        ...
        const cleaningData = { id: cleaningSnap.id, ...(cleaningSnap.data() as Record<string, any>) } as CleaningData;

        // Carica la proprietà UNA SOLA VOLTA e la mette in cache
        if (cleaningData.propertyId && !propertyCache.current) {
          try {
            const propertyRef = doc(db, "properties", cleaningData.propertyId);
            const propertySnap = await getDoc(propertyRef);

            if (propertySnap.exists()) {
              propertyCache.current = { id: propertySnap.id, ...(propertySnap.data() as Record<string, any>) } as PropertyData;
            }
          } catch (err) {
        ...
        setCleaning(prev => ({
          ...cleaningData,
          property: propertyCache.current || prev?.property
        }));
        setLoading(false);
      },
      (err) => {
  ...
  // ── Verifica che l'operatore sia ancora assegnato a questa pulizia ──
  ...
  if (!isAdmin && !isAssigned && cleaning.status !== "COMPLETED") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-sky-50/30 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-2">Non sei più assegnato</h2>
          <p className="text-slate-500 mb-6">Questa pulizia è stata riassegnata a un altro operatore.</p>
          <Link href="/operatore" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl font-medium hover:shadow-lg transition-all">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
