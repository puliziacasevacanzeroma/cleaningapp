...
export default async function PrenotazioneDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (user.role?.toUpperCase() === "PROPRIETARIO") {
    ...
    if (!propertySnap.exists() || (propertySnap.data() as Record<string, any>).ownerId !== user.id) {
      notFound();
    }
  }
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          ...
          <p className={`text-xl font-bold ${(booking as any).isPaid ? "text-emerald-600" : "text-amber-600"}`}>
            {(booking as any).isPaid ? "Pagato" : "In attesa"}
          </p>
        ...
        <div className="mb-6 p-4 rounded-2xl border bg-amber-50 border-amber-200">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700">Il numero di ospiti può essere modificato <strong>entro mezzanotte del giorno del check-out</strong>. Dopo questo termine, la biancheria sarà preparata per il numero massimo di ospiti della proprietà ({(booking as any).property?.maxGuests}).</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">{(booking as any).guestsCount ? `${(booking as any).guestsCount} ospiti confermati` : "Inserisci il numero di ospiti"}</p>
          </div>
          {(booking as any).guestsCount && (
            <span className="text-4xl font-bold text-slate-800">{(booking as any).guestsCount}</span>
          )}
        </div>
        {canModifyGuests && (
          <div className="mt-4 pt-4 border-t border-slate-100">
            <GuestCountForm bookingId={booking.id} currentGuests={(booking as any).guestsCount} maxGuests={(booking as any).property?.maxGuests || 6} />
          </div>
        )}
      </div>
