...
export function useContractCheck(): ContractCheckResult {
  // @ts-expect-error TODO-FIX: TS2339 Property 'userData' does not exist on type 'AuthContextType'.
  ...
  const checkContract = useCallback(async () => {
    ...
    try {
      ...
      // @ts-expect-error TODO-FIX: TS2339 Property 'getIdToken' does not exist on type 'AuthUser'.
      ...
      if (userData) {
        saveUserInfoCookie({
          // @ts-expect-error TODO-FIX: TS2339 Property 'uid' does not exist on type 'AuthUser'.
          uid: user.uid,
          role: userData.role || "",
